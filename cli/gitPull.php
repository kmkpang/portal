<?php
/**
 * @package        Joomla.Site
 * @copyright    Copyright (C) 2005 - 2012 Open Source Matters, Inc. All rights reserved.
 * @license        GNU General Public License version 2 or later; see LICENSE.txt
 */
if (!defined('_JEXEC'))
    define('_JEXEC', 1);

define('__ISUNITTEST', false);

error_reporting(1);


define('DS', DIRECTORY_SEPARATOR);

ini_set('display_errors', '1'); // only for xampp , because it screws up the display
ini_set('max_execution_time', '0');
ini_set('memory_limit', '2048M'); // required only on linux and ubuntu !

// We are a valid entry point.
const _JEXEC = 1;

// Load system defines
if (file_exists(dirname(__DIR__) . '/defines.php')) {
    require_once dirname(__DIR__) . '/defines.php';
}

if (!defined('_JDEFINES')) {
    define('JPATH_BASE', dirname(__DIR__));
    require_once JPATH_BASE . '/includes/defines.php';
}

/**
 * Constant that is checked in included files to prevent direct access.
 * define() is used in the installation folder rather than "const" to not error for PHP 5.2 and lower
 */
define('_JEXEC', 1);

if (file_exists(__DIR__ . '/defines.php')) {
    include_once __DIR__ . '/defines.php';
}

if (!defined('_JDEFINES')) {
    define('JPATH_BASE', __DIR__);
    require_once JPATH_BASE . '/includes/defines.php';
}
// Get the framework.
require_once JPATH_LIBRARIES . '/import.legacy.php';

// Bootstrap the CMS libraries.
require_once JPATH_LIBRARIES . '/cms.php';
// Framework import
require_once JPATH_BASE . '/includes/framework.php';
require_once JPATH_BASE . '/cliParser.php';

///var/www/softverk-webportal-remaxth/libraries/webportal/services/webservice/websending/websendingBase.php

require_once JPATH_BASE . '/libraries/webportal/services/webservice/websending/websendingBase.php';

// Instantiate the application.
$app = JFactory::getApplication('site');

ob_start(); // Start output buffering

// Execute the application.
$app->execute();

$list = ob_get_contents(); // Store buffer in variable

ob_end_clean(); // End buffering and clean up

/**
 * This script will fetch the update information for all extensions and store
 * them in the database, speeding up your administrator.
 *
 * @package  Joomla.Cli
 * @since    2.5
 */
class Gitpull extends JApplicationCli
{

    var $files_to_bkup = array(
        "Grunt-settings.json",
        "favicon.ico",
        "images/logo.png",
        "images/favicon.ico",
        "webportal.configuration.js",
        "webportal.configuration.php",
        "configuration.php",
        "images/carousel_bg"
    );


    /**
     * Entry point for the script
     *
     * @return  void
     *
     * @since   2.5
     */
    public function doExecute()
    {

        $gitInfo = WFactory::getHelper()->getVersionInfo();

        $branch = $gitInfo['branch'];
        $this->backupFiles($branch);
        $this->resetBranch();
        $this->pull($branch);
        $this->restoreFiles($branch);
        $this->deleteFiles($branch);
        $this->restorePermission($branch);
        $this->writeRobotsText();

        WFactory::getLogger()->info("********** done : " . WFactory::getHelper()->getLastCommit() . " ************");


    }

    public function backupFiles($branchName)
    {
        foreach ($this->files_to_bkup as $f) {
            $fileName = explode("/", $f);
            $fileName = $fileName[count($fileName) - 1];
            $result = copy(JPATH_ROOT . "/$f ", JPATH_BASE . "/tmp/$fileName");
//            if (!$result) {
//                WFactory::getLogger()->warn("Failed to backup file from " . JPATH_ROOT . "/$f ==> " . JPATH_BASE . "/tmp/$fileName" . " , exiting..");
//                exit(1);
//            }

            $command = "sudo cp -rf " . JPATH_ROOT . "/$f " . JPATH_BASE . "/tmp/$fileName";
            $retCode = 0;
            $this->executeShellCommand($command, $retCode, false);
        }

    }

    public function restoreFiles($branchName)
    {
        foreach ($this->files_to_bkup as $f) {
            $fileName = explode("/", $f);
            $fileName = $fileName[count($fileName) - 1];

            $result = copy(JPATH_BASE . "/tmp/$fileName", JPATH_ROOT . "/$f ");
//            if (!$result) {
//                WFactory::getLogger()->warn("Failed to backup file from : " . JPATH_BASE . "/tmp/$fileName" . " ==>  " . JPATH_ROOT . "/$f exiting..");
//                exit(1);
//            }
//
            $command = "sudo cp -rf " . JPATH_BASE . "/tmp/$fileName " . JPATH_ROOT . "/$f ";
            $retCode = 0;
            $this->executeShellCommand($command, $retCode, false);
        }
    }

    public function deleteFiles($branchName)
    {
        foreach ($this->files_to_bkup as $f) {
            $fileName = explode("/", $f);
            $fileName = $fileName[count($fileName) - 1];

            $command = "sudo rm -rf " . JPATH_BASE . "/tmp/$fileName ";
            $retCode = 0;
            $this->executeShellCommand($command, $retCode, false);
        }
    }

    private function resetBranch()
    {
        $command = "git reset --hard";
        $retCode = 0;
        $result = $this->executeShellCommand($command, $retCode);
    }

    private function pull($branch)
    {
        $command = "git pull origin $branch";
        $retCode = 0;
        $result = $this->executeShellCommand($command, $retCode);
    }

    private function restorePermission($branch)
    {
        $command = "sudo chown -R www-data:www-data ../";
        $retCode = 0;
        $result = $this->executeShellCommand($command, $retCode);
    }

    private function executeShellCommand($command, &$returnValue, $exit = true)
    {
        $output = array();

        exec($command, $output, $returnValue);
        WFactory::getLogger()->debug("$command ($returnValue)-> " . implode("\n", $output));


        if ($returnValue !== 0) {
            WFactory::getLogger()->fatal("Last command : $command has return value : $returnValue , aborting..! fix this manually and start over!");

            if ($exit)
                exit(1);
        }

        return implode("\n", $output);
    }

    private function writeRobotsText()
    {
        $robots = <<<EOD
User-agent: *
DisaLlow: /administrator/
DisaLlow: /bin/
DisaLlow: /cache/
DisaLlow: /cli/
DisaLlow: /components/
DisaLlow: /includes/
DisaLlow: /installation/
DisaLlow: /language/
DisaLlow: /layouts/git
DisaLlow: /libraries/
DisaLlow: /logs/
DisaLlow: /media/
DisaLlow: /modules/
DisaLlow: /plugins/
DisaLlow: /templates/
DisaLlow: /tmp/
DisaLlow: /sql/
DisaLlow: /sample/
DisaLlow: /vendor/
DisaLlow: /build/
DisaLlow: /.git/

Allow : /assets/
Allow : /templates/webportal/js/

Sitemap: http://sales.softverk.co.th/sitemap.xml

EOD;

        $robots = str_replace("sales.softverk.co.th", __SITEURL,$robots);

        print_r($robots);
        $robotPath=JPATH_BASE . "/robots.txt";
        echo "\n\n---copying to : $robotPath\n\n";
        file_put_contents($robotPath, $robots);



    }

}


JApplicationCli::getInstance('Gitpull')->execute();
