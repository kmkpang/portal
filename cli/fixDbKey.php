 ...
 * @since    2.5
 */
class Fixdbkey extends JApplicationCli
{
    /**
    ...
    public function doExecute()
    {
        ...
        foreach ($databases as $database) {
            ...
            if (strpos($database, 'portal-') !== false) {
                ...
                $tables = WFactory::getSqlService()->select($query);

                foreach ($tables as $table) {
                    $table = $table["Tables_in_$database"];
                    if (strpos($table, "jos_portal_") !== false) {
                        WFactory::getLogger()->info("Fixing fucking $table");
                        $query = "ALTER TABLE  `$database`.`$table` CHANGE  `id`  `id` INT( 11 ) NOT NULL AUTO_INCREMENT ;";
                        $result = WFactory::getSqlService()->update($query);

                        WFactory::getLogger()->info("$query --> $result");
                    }
                }
...
JApplicationCli::getInstance('Fixdbkey')->execute();
