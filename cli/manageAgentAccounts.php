 ...
 * @since    2.5
 */
class ManageAgentAccounts extends JApplicationCli
{
    /**
    ...
    public function doExecute()
    {
        ...
        WFactory::getLogger()->info("creating agents from jos_portal_sales");
        $query = 'select id from jos_portal_sales';
        $result = WFactory::getSqlService()->select($query);

        foreach ($result as $r) {
            WFactory::getServices()->getServiceClass(__PROPPERTY_PORTAL_AGENT)->processAgentAccount($r['id']);
        }
...
JApplicationCli::getInstance('ManageAgentAccounts')->execute();
