...
class PropertyDeleteSentToWebTest extends PHPUnit_Framework_TestCase
{
    /**
...
//        $query = "SELECT data  FROM `jos_portal_senttoweb_log` WHERE `command` LIKE 'Delete' AND `type` LIKE 'Property' order by id desc LIMIT 1";
//        $result = WFactory::getServices()->getSqlService()->select($query);
//        $xml = $result[0]['data'];
//        $xml = new SimpleXMLElement($xml);

        ///var/www/softverk-webportal-remaxth/tests/portaltest/testXml/porpertyDelete.xml.xsl

        $xml = file_get_contents(JPATH_BASE . DS . "tests/portaltest/testXml/propertyDelete.xml.xsl");
        $xml = new SimpleXMLElement($xml);
...
//        $xml->Properties->Property->Information->SaleID = $agentId;
    ...
    public function testDelete()
    {
        ...
        $result = ($this->object->delete($sent2webDbClass));
        ...
        $this->assertTrue($xmlData["Response"]["Number"] == "02131");
    }

    public function testDeleteWebService()
    {
