...
class PropertyDeleteSentToWebTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        if ($quickTest) {
            ...
            $xml = file_get_contents('/var/www/softverk-webportal/tests/portaltest/testXml/propertyDelete.xml.xsl');
        } else {
            ...
            $query = "SELECT data  FROM `jos_portal_senttoweb_log` WHERE `command` LIKE 'Delete' AND `type` LIKE 'Property'";
            $result = WFactory::getServices()->getSqlService()->select($query);
            $xml = $result[0]['data'];
            $xml = new SimpleXMLElement($xml);
           ...
           // file_put_contents('/var/www/softverk-webportal/tests/portaltest/testXml/propertyDelete.xml.xsl', $xml);
        }
        $this->xmlString = $xml;
        $this->object = new PropertySentToWeb($xml);
    ...
    public function testDelete()
    {
        ...
        $this->assertTrue($xmlData["Response"]["Number"] == "02231");
    }
