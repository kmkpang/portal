...
class PropertyDetailsModel extends ModelBase
{
    ...
    var $description_text_without_html;
...
//        if (strpos($this->address, '/th') !== false || strpos($this->address, '/en') !== false) {
//            $this->address = WFactory::getServices()->getServiceClass(__PROPPERTY_PORTAL_PROPERTY)->getAddressByLanguage($this->property_id, $lang);
//        }

        $images = WFactory::getServices()->getServiceClass(__PROPPERTY_PORTAL_PROPERTY)->getAllPropertyImages($this->property_id);

        if ($images[0]) {
            $this->list_page_thumb_path = $images[0]->__server_url;
        }
        ...
        if (!WFactory::getHelper()->isNullOrEmptyString($this->region_name)) {
            ...
            $result = WFactory::getSqlService()->select("Select * from jos_geography_regions where id={$array['region_id']}");
            if ($result[0])
                $result = $result[0];

            $this->region_name = $result["name_$lang"];
...
//            }
        ...
        if (!WFactory::getHelper()->isNullOrEmptyString($this->city_town_name)) {
            $result = WFactory::getSqlService()->select("Select * from jos_geography_towns where id=${array['city_town_id']}");
            if ($result[0])
                $result = $result[0];

            $this->city_town_name = $result["name_$lang"];
...
//            }
        ...
        if (!WFactory::getHelper()->isNullOrEmptyString($this->zip_code_name)) {
            $result = WFactory::getSqlService()->select("Select * from jos_geography_postal_codes where id={$array['zip_code_id']}");
            if ($result[0])
                $result = $result[0];

            $this->zip_code_name = $result["name_$lang"];
...
//            }
        ...
        //}
        $this->description_text = WFactory::getServices()->getServiceClass(__PROPPERTY_PORTAL_PROPERTY)->getDescriptionByLanguage($this->property_id, $lang);
        $this->description_text_without_html = strip_tags($this->description_text);
...
//for a start, take the creation date
        ...
        //Temporary fix in Thailand
        if (__COUNTRY == "TH") {

            if (!empty($this->land_area)) {
                $this->living_area = $this->total_area;
                $this->total_area = $this->land_area;
            } else {
                $this->total_area = $this->total_area;
            }
        }
        ...
        } else if ($this->is_featured == 1) {
            $this->is_featured = true;
        }

        //is_investment
        if ($this->is_investment == 0) {
            $this->is_investment = false;
        } else if ($this->is_investment == 1) {
            $this->is_investment = true;
        }
    }
