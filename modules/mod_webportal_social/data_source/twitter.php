...
class SocialTwitterHelper
{
    ...
    function getData()
    {
        ...
        if($this->config['twitter_use_css']) {
            ...
            $doc->addStyleSheet( $uri->root().'modules/mod_webportal_social/styles/twitter/'.$this->config['twitter_tweet_style'].'.css', 'text/css' );
        }
    
        if($this->config['twitter_cache'] == 1) {
            if(filesize(realpath('modules/mod_webportal_social/cache/cache.'.$this->id.'.json')) == 0 || ((filemtime(realpath('modules/mod_webportal_social/cache/cache.'.$this->id.'.json')) + $this->config['twitter_cache_time'] * 60) < time())) {
            ...
            if($this->error == '') {
                // saving cache
                if($this->pData != '') {
                	JFile::write(realpath('modules/mod_webportal_social/cache/cache.'.$this->id.'.json'), json_encode($this->pData));
                	JFile::write(realpath('modules/mod_webportal_social/cache/cache.backup.'.$this->id.'.json'), json_encode($this->pData));
                }
            } else {
                if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
                    $this->pData = json_decode(JFile::read(realpath('modules/mod_webportal_social/cache/cache.backup.'.$this->id.'.json')), true, 512, JSON_BIGINT_AS_STRING);
                } else {
                    $this->pData = json_decode(JFile::read(realpath('modules/mod_webportal_social/cache/cache.backup.'.$this->id.'.json')));
                }
            }
            } else {
                if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
                    $this->pData = json_decode(JFile::read(realpath('modules/mod_webportal_social/cache/cache.'.$this->id.'.json')), true, 512, JSON_BIGINT_AS_STRING);
                } else {
                    $this->pData = json_decode(JFile::read(realpath('modules/mod_webportal_social/cache/cache.'.$this->id.'.json')));
                }
            } /// close
        } else {
            $this->getTweets(); 
     ...
     **/
    function render() {
        require (JModuleHelper::getLayoutPath('mod_webportal_social', 'twitterTweets'));
    }
